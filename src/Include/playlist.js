...
const {createQuery} = require('../Functions/musicFunctions');
const {play} = require('./play');
const ytpl = require('ytpl');

module.exports = {
  /**
   * Load playlist url to the queue
  ...
  async playlist(message, args) {
    ...
    if (member.voice.deaf) {
      ...
      return await message.reply({
            embeds: [
              new Embed().setDescription(
                  'You can\'t run this command while deafened!').
                  setColor('RED')],
          },
      );
    }
    ...
    if (!permissions.has('CONNECT'))
      return channel.reply(
          ':x: I couldn\'t connect to the voice channel, missing permissions.');
    ...
    if (urlValid) {
      ...
      return message.reply({
            embeds: [
              new Embed().setDescription('Invalid Playlist URL').
                  setColor('ORANGE')],
          },
      );
    }

    const newSongs = videos.filter(
        (video) => video.title != 'Private video' && video.title !=
            'Deleted video').map((video) => {
    ...
    serverQueue
        ...
        : queueConstructor.songs.push(...newSongs);

    let playlistEmbed = new Embed().setTitle(`${playlist.title}`).
        setDescription(newSongs.map(
            (song, index) => `${index + 1}. ${song.fullTitle}`)).
        setURL(playlist.url).
        setColor('#F8AA2A').
        setTimestamp();
    ...
    if (!serverQueue) {
      ...
      try {
        ...
        await play(queueConstructor.songs[0], message);
      } catch (error) {
        console.error(error);
        ...
        return channel.send(
            new Embed().setDescription(
                `I Couldn't join to the voice channel: ${error}`).
                setColor('RED'),
        ).catch((error) => console.error(error));
      }
    }
