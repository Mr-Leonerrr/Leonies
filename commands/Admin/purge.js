  ...
  callback: async (message) => {
    ...
    if (!message.member.hasPermission("ADMINISTRATOR")) {
      ...
      try {
        ...
        return message.reply("Enter the number of messages to delete. (max 100)");
      }
      ...
      message.channel
        .bulkDelete(fetched)
        .catch((error) => message.reply(`Messages could not be deleted due to: ${error}`));
      message
        .reply(`\`${fetched.size}\` Messages have been deleted successfully 💥.`)
        .then((msg) => {
